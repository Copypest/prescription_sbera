...
class admin{
	...
	function getUserDetails($user_id){
        ...
        $result = mysqli_query($this->conn,$_QUERY) or die(mysqli_error($this->conn));
    ...
    function insertUpdatePatientInvestigation($investigation_name, $type, $unit, $value, $patient_id, $visit_id,$chamber_name,$doc_name){
        ...
        $result = mysqli_query($this->conn,$query_getinvestigation_details_from_master) or die(mysqli_error($this->conn));
        if (mysqli_num_rows($result) > 0){
            //Investigation exists in Master. Only insert into patient_investigation table
            $rowresult = mysqli_fetch_object($result) or die(mysqli_error($this->conn));
            //Get the investigation Id
            $inv_id = $rowresult->ID;
            
            //update investigation master
            mysqli_query($this->conn,"update investigation_master a set  a.unit = '$unit' where a.ID ='$inv_id' AND a.chamber_id='$chamber_name' AND a.doc_id='$doc_name'" ) or die(mysqli_error($this->conn));
        ...
        //INsert into patient_investigation
            ...
            mysqli_query($this->conn,$query_insert_into_patient_investigation) or die(mysqli_error($this->conn));
        } else {
            ...
            mysqli_query($this->conn,$query_insert_into_investigation_master) or die(mysqli_error($this->conn));
            //Get the investigation Id
            //$inv_id = mysqli_insert_id($this->conn) or die(mysqli_error($this->conn));
    ...
    function deletePatientInvestigation($investigation_id,$visit_id,$chamber_name,$doc_name ){
    	...
    	//echo $deletQuery;
    	mysqli_query($this->conn,$deletQuery) or die(mysqli_error($this->conn));
        ...
        $investigation_name = $investigation->investigation_name;
         ...
         if(strtoupper($investigation_name) == "CREATININE" ){
            ...
            $result12 = mysqli_query($this->conn,$_QUERY12)or die(mysqli_error($this->conn));
            $ci_id = "";
    ...
    function deleteCF($mode, $cf_id, $visit_id, $cfvalue,$chamber_name,$doc_name){
        ...
        if($mode == 'UPDATE'){
            ...
            mysqli_query($this->conn,"update patient_health_details a
                        set a.VALUE = '$cfvalue' where a.VISIT_ID = '$visit_id' 
                        and a.ID  ='$cf_id' and a.chamber_id='$chamber_name' and a.doc_id='$doc_name'") or die(mysqli_error($this->conn));
                if (mysqli_affected_rows($this->conn) > 0){
                    $result =  "<tr><td colspan='3'>". mysqli_affected_rows($this->conn) ." item(s) updated</td></tr>";
                }

            } else if($mode == 'DELETE'){
                mysqli_query($this->conn,"delete from patient_health_details 
                        where VISIT_ID = '$visit_id' 
                        and ID  ='$cf_id' and chamber_id='$chamber_name' and doc_id='$doc_name'") or die(mysqli_error($this->conn));
                if (mysqli_affected_rows($this->conn) > 0){
                    $result =  "<tr><td colspan='3'>". mysqli_affected_rows($this->conn) ." item(s) deleted</td></tr>";
                }
            }

        if($cf_id == '1' || $cf_id = '2'){
            //Modify BMI
            $result1 = mysqli_query($this->conn,"select a.VALUE from patient_health_details a 
                    where a.ID = '1' and a.VISIT_ID = '$visit_id' and a.chamber_id='$chamber_name' and a.doc_id='$doc_name'") or die(mysqli_error($this->conn));

            if(mysqli_num_rows($result1) > 0){
                $obj = mysqli_fetch_object($result1);
                $height = $obj->VALUE;
            }
            $result2 = mysqli_query($this->conn,"select a.VALUE from patient_health_details a 
                    where a.ID = '2' and a.VISIT_ID = '$visit_id' and a.chamber_id='$chamber_name' and a.doc_id='$doc_name'") or die(mysqli_error($this->conn));
            ...
            if($height != "" && $weight != ""){
                ...
                $result_id_f = mysqli_query($this->conn,"select * from patient_health_details a where a.ID = '3' AND a.chamber_id='$chamber_name' AND a.doc_id='$doc_name'") or die(mysqli_error($this->conn));
                ...
                mysqli_query($this->conn,$query_b) or die(mysqli_error($this->conn));
            }
        }
        return $result;
    }
    ...
    function insertUpdateCF($cfname,$cfvalue,$visit_id,$chamber_name,$doc_name){
        ...
        if($height != "" && $weight != ""){
            $bmi = $admin->calcBMI($weight, $height);
            $result_id_f = mysqli_query($this->conn,"select * from patient_health_details a where 
                a.ID = '3' and a.VISIT_ID = '$visit_id' AND a.chamber_id='$chamber_name' AND a.doc_id='$doc_name'") or die(mysqli_error($this->conn));
            if(mysqli_num_rows($result_id_f) > 0 ){
            ...
            $result_ideal_body_weight = mysqli_query($this->conn,"select * from patient_health_details a,  patient_health_details_master b where
														a.ID = b.ID and a.chamber_id = b.chamber_id and a.doc_id = b.doc_id 
														and a.chamber_id='$chamber_name' and a.doc_id='$doc_name'
														and a.VISIT_ID = '$visit_id' and b.NAME = 'Ideal Body Weight (KG)'") or die(mysqli_error($this->conn));
            ...
            //echo $query_ideal_body_weight;
            mysqli_query($this->conn,$query_ideal_body_weight) or die(mysqli_error($this->conn));
        }
    ...
    function deleteClinicalImpression($prescription_id,$ci_id, $chamber_name, $doc_name){
    	...
    	//echo $_QUERY;
    	mysqli_query($this->conn,$_QUERY) or die(mysqli_error($this->conn));
    ...
    function deleteSocialHistory($prescription_id,$ci_id, $chamber_name, $doc_name){
        ...
        mysqli_query($this->conn,"delete from prescribed_social_history
             where prescription_id = '$prescription_id'
             and social_history_id  ='$ci_id' and chamber_id='$chamber_name' AND doc_id='$doc_name'") or die(mysqli_error($this->conn));
        
    }
    function deleteAllergy($prescription_id,$allergy_id, $chamber_name, $doc_name){
        //$message = "";
        mysqli_query($this->conn,"delete from prescribed_allergy
             where prescription_id = '$prescription_id'
             and ALLERGY_ID  ='$allergy_id' and chamber_id='$chamber_name' AND doc_id='$doc_name'") or die(mysqli_error($this->conn));
    }
    ...
    function deletePastMedicalHistory($prescription_id,$ci_id, $chamber_name, $doc_name){
        //$message = "";
        mysqli_query($this->conn,"delete from prescribed_past_med_history
             where prescription_id = '$prescription_id'
             and clinical_impression_id  ='$ci_id' and chamber_id='$chamber_name' AND doc_id='$doc_name'") or die(mysqli_error($this->conn));
    ...
    function insertintoDoseMasterTable ($dose, $chamber_name, $doc_name){
    	...
    	//echo $query_insert;
    	$result = mysqli_query($this->conn,$query_search) or die(mysqli_error($this->conn));
    	$num_res = mysqli_num_rows($result);
    	//echo "num_res=".$num_res;
    	 if ($num_res<= 0){
    		//Insert into dose_details_master
    		//echo "Exceuted the query";
    		mysqli_query($this->conn,$query_insert) or die(mysqli_error($this->conn));
    	} 
    ...
    function insertUpdateAllergy($PRESCRIPTION_ID, $ALLERGY,$chamber_name,$doc_name){
        ...
        $query1 = "insert into prescribed_allergy(allergy_id, prescription_id,chamber_id, doc_id)
                    values('$id' , '$PRESCRIPTION_ID','$chamber_name','$doc_name')";
        mysqli_query($this->conn,$query1) or die(mysqli_error($this->conn));
    }
