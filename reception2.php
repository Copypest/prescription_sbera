...
$r2 = mysqli_query($link,"select * from visit where PATIENT_ID = '$patient_id'") or die(mysqli_error($link));
...
$r0 = mysqli_query($link,$sql0) or die(mysqli_error($link));
$n0 = mysqli_num_rows($r0);
if($n0 < 1){
	$d2 = mysqli_fetch_assoc($r2) or die(mysqli_error($link));
	mysqli_query($link,"insert into patient_health_details_by_receptionist (patient_id, VISIT_ID) values('$patient_id','$d2[VISIT_ID]')");
}

$sql1 = mysqli_query($link,"select * from patient where patient_id = '$patient_id'") or die(mysqli_error($link));
$d1 = mysqli_fetch_object($sql1) or die(mysqli_error($link));
...
$d2 = mysqli_fetch_object($result2) or die(mysqli_error($link));
...
$d3 = mysqli_fetch_assoc($r3) or die(mysqli_error($link));
