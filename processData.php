...
if(isset($_GET['patient_id'])){
    ...
    $result = mysqli_query($link,"select * from visit where patient_id = '$patient_id' and doc_id =  '$doc_name' and chamber_id = '$chamber_name' and visited = 'no'");
    ...
    if(mysqli_num_rows($result) > 0){
        ...
        mysqli_query($link,"insert into visit (VISIT_ID, patient_id,VISIT_DATE,chamber_id, created_by_user_id, doc_id ) 
            values( '".$existing_visit_id."', '".$patient_id."', NOW(),'".$chamber_name."','".$user_name."','".$doc_name."' )") or die(mysqli_error($link));
        //$existing_visit_id = mysqli_insert_id($link);
        //echo "System Creats visit id = ".$existing_visit_id;
    }
    /** END : BLOCK FOR GENERATING VISIT ID   **/
    
    //Get the last visited record
    $result1 = mysqli_query($link,"select max(visit_id) as visit_id from visit where patient_id = '".$patient_id."' and visited = 'yes' and doc_id =  '$doc_name' and chamber_id = '$chamber_name'" );
    ...
    if(mysqli_num_rows($result1) > 0){
        ...
        $result2 = mysqli_query($link,"select * from patient_investigation where patient_id  = '".$patient_id."'and visit_id='".$visited_id."' and doc_id =  '".$doc_name."' and chamber_id = '".$chamber_name."'");
        
        while($existingRows = mysqli_fetch_assoc($result2)){
        ...
        $result3 = mysqli_query($link,"select * from patient_health_details where visit_id='".$visited_id."' and doc_id =  '".$doc_name."' and chamber_id = '".$chamber_name."'") or die(mysqli_error($link));
        
        while($existingRows1 = mysqli_fetch_assoc($result3)){
            ...
            if($id == 1 ){
                ...
                $result4 = mysqli_query($link,$checkQuery2);
                if(mysqli_num_rows($result4) <= 0) {
                    ...
                    mysqli_query($link,$query_insert_into_patient_health_details) or die(mysqli_error($link));
                }
            }
